    ...
    private displayPairHistory = async (): Promise<void> => {
        ...
        pairList.forEach((pair, index) => {
            if (pair.lastDate?.getMilliseconds() === new Date(-8640000000000000).getMilliseconds()) {
                console.log(`     ${index + 1}: ${pair['first']}, ${pair['second']} , None`);
            } else {
                console.log(`     ${index + 1}: ${pair['first']}, ${pair['second']} , ${pair.lastDate}`);
            }
        });
    };

    private makePair = async () => {
        const pairs = await this.team.makePairMob(this.repo.load());

        console.log('');
        console.log(`Today's pair:`);
        pairs.forEach((pair, index) => {
            if (pair['third']) {
                console.log(`     ${index + 1}: ${pair['first']}, ${pair['second']}, ${pair['third']}`);
            } else {
                console.log(`     ${index + 1}: ${pair['first']}, ${pair['second']}`);
            }
